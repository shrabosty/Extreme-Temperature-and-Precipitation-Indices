...
TMAX_PATH = r"C:\Ovi\Bangladesh\ACCESS-CM2\ssp245\TMaxData"
TMIN_PATH = r"C:\Ovi\Bangladesh\ACCESS-CM2\ssp245\TMinData"
PRCP_PATH = r"C:\Ovi\Bangladesh\ACCESS-CM2\ssp245\PrecipData"
OUT_DIR   = r"C:\Ovi\Bangladesh\ACCESS-CM2\ssp245"

WETDAY_THRESHOLD = 1.0  # mm
SU25_THRESHOLD   = 25.0 # °C
TR20_THRESHOLD   = 20.0 # °C
RUN_LEN          = 5    # WSDI/CSDI
ROLL_WIN_RX5     = 5    # Rx5day window
...
def annual_agg(vals_df, years_vec, func):
    ...
    for y in np.unique(years_vec):
        ...
        agg.name = y
        out.append(agg)
    return pd.DataFrame(out).mean(axis=0, skipna=True)
...
def mean_wsdi_like_per_point(temps_b, years_vec, threshold_series, cmp="gt"):
    ...
    for y in np.unique(years_vec):
        ...
        is_ext = year_vals.gt(threshold_series, axis=1) if cmp == "gt" else year_vals.lt(threshold_series, axis=1)
        for c in temps_b.columns:
            arr = is_ext[c].to_numpy()
            if np.isfinite(year_vals[c]).sum() == 0:
                continue
            totals[c] += total_days_in_runs_ge_k(arr, RUN_LEN)
            n_years[c] += 1
    return pd.Series({c: (totals[c] / n_years[c]) if n_years[c] > 0 else np.nan for c in temps_b.columns})
...
def mean_annual_count(vals_b, years_vec, threshold, cmp="gt"):
    ...
    for y in np.unique(years_vec):
        idx = (years_vec == y)
        v = vals_b.loc[idx, :]
        cnt = (v.gt(threshold) if cmp == "gt" else v.ge(threshold)).sum(axis=0)
        cnt.name = y
        yearly.append(cnt)
...
def rx5day_mean(pr_b, years_vec, win=5):
    ...
    for y in np.unique(years_vec):
        ...
        max5 = roll.max(axis=0); max5.name = y
        yearly.append(max5)
    return pd.DataFrame(yearly).mean(axis=0, skipna=True)
...
def rXXp_mean(pr_b, years_vec, pthr_series, use_strict=True):
    ...
    for y in np.unique(years_vec):
        ...
        s = g.where(mask).sum(axis=0, min_count=1); s.name = y
        yearly.append(s)
    return pd.DataFrame(yearly).mean(axis=0, skipna=True)
...
def rNmm_mean(pr_b, years_vec, threshold_mm):
    ...
    for y in np.unique(years_vec):
        idx = (years_vec == y)
        g = pr_b.loc[idx, :]
        c = (g >= threshold_mm).sum(axis=0); c.name = y
        yearly.append(c)
    return pd.DataFrame(yearly).mean(axis=0, skipna=True)
...
def mean_annual_max_run(pr_b, years_vec, threshold, mode="dry"):
    ...
    for y in np.unique(years_vec):
        idx = (years_vec == y)
        g = pr_b.loc[idx, :]
        bool_df = (g.lt(threshold) if mode == "dry" else g.ge(threshold)) & g.notna()
...
def prcptot_mean(pr_b, years_vec, wet_thr=1.0):
    ...
    for y in np.unique(years_vec):
        idx = (years_vec == y)
        g = pr_b.loc[idx, :]
        wet_sum = g.where(g >= wet_thr).sum(axis=0, min_count=1); wet_sum.name = y
        yearly.append(wet_sum)
    return pd.DataFrame(yearly).mean(axis=0, skipna=True)

def rXXpTOT_mean_percent(pr_b, years_vec, pthr_series, wet_thr=1.0, strict=True):
    pct_years = []
    for y in np.unique(years_vec):
        idx = (years_vec == y)
        g = pr_b.loc[idx, :]
...
def compute_period(base_start, base_end, out_csv):
    # Masks
    ...
    tx_tn_diff = (tx_b.to_numpy() - tn_b.to_numpy())
    dtr_yearly = []
    ...
    R95PTOT= rXXpTOT_mean_percent(pr_b, years_pr, pr_p95, WETDAY_THRESHOLD, strict=True)
    R99PTOT= rXXpTOT_mean_percent(pr_b, years_pr, pr_p99, WETDAY_THRESHOLD, strict=True)
    ...
    print(f"Saved indices for {base_start}-{base_end} to: {out_csv}")
    return out

compute_period(2015, 2039, fr"{OUT_DIR}\All_Indices_access_cm2_2015_2039.csv")
compute_period(2040, 2069, fr"{OUT_DIR}\All_Indices_access_cm2_2040_2069.csv")
compute_period(2070, 2100, fr"{OUT_DIR}\All_Indices_access_cm2_2070_2100.csv")
